...
modelpath = os.path.join(PROJECT_PATH,'models','citiesKira')
resultpath = os.path.join(PROJECT_PATH,'results','citiesKira')


# Logging setup
logging.basicConfig(filename=os.path.join(resultpath,'vae_cnn2.log'), filemode='w', format='%(message)s')
...
model.load_state_dict(torch.load(os.path.join(modelpath,'vae.torch'), map_location='cpu'))   # Start with pretrained model.
...
def train(epoch):
    ...
    torch.save(model.state_dict(), os.path.join(modelpath,'vae.torch'))
